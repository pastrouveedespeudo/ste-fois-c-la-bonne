...
class outils_internet: 
    ...
    def recherche_image(self, mot_cle, mot_cle1, liste1):
    ...
    def recherche_image_phrase(self, phrase, liste1):
    ...
    def recherche_image_verbe(self, mot_cle, liste1):
    ...
    def recherche_langage(self, recherche, liste):

        self.recherche = recherche

        self.liste = liste

        path = "https://fr.wiktionary.org/wiki/{}".format(self.recherche)
        requete = requests.get(path)
        page = requete.content
        soup = BeautifulSoup(page, "html.parser")  
        for ol in soup.find_all("ol"):
            self.liste.append(ol.text)
